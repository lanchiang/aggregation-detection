...
class SumQualityEval(QualityEval):
    ...
    def extract_single_type_ground_truth(self):
        return {(file_dict['file_name'], file_dict['table_id']):
                    [annotation for annotation in file_dict['aggregation_annotations'] if annotation['operator'] == AggregationOperator.SUM.value]
                for file_dict in self.file_dicts}

    def extract_single_type_predictions(self):
        return {key: [prediction for prediction in predictions if prediction['operator'] == AggregationOperator.SUM.value]
    ...
    def _match_ground_truth(self):
        for file_id, file_gt in self.ground_truth.items():
            ...
            for gt in file_gt:
                ...
                match = bool(
                    [p for p in self.single_type_predictions[file_id] if is_aggregation_equal((gt_aggregator, gt_aggregatees, AggregationOperator.SUM.value),
        ...
        for file_id, file_predictions in self.single_type_predictions.items():
            ...
            for prediction in file_predictions:
                ...
                match = bool([gt for gt in self.ground_truth[file_id] if
                              is_aggregation_equal((gt['aggregator_index'], gt['aggregatee_indices'], AggregationOperator.SUM.value),
                                                   (pred_aggregator, pred_aggregatees, AggregationOperator.SUM.value),
                                                   self.file_values[file_id])])
