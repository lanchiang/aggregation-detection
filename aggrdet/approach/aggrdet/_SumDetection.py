...
class SumDetection(MultiAggregateeAggrdet):
    ...
    def run(self):
        ...
        with self.output().open('w') as file_writer:
            for file_output_dict in tqdm(files_dict_map.values(), desc='Serialize sum results'):
    ...
    def sum_equal(self, aggregator_value, aggregatees, based_aggregator_value, error_bound):
        expected_average = sum(aggregatees)
                ...
                for aggregation in sorted_aggregations:
                    ...
                    for row_index in valid_row_indices:
                        ...
                        if not all([n is not None for n in numberized_aggee_values]) or not possible_aggor_value:
                            continue
                        real_error_level = self.sum_equal(possible_aggor_value, numberized_aggee_values, based_aggregator_value, error_bound)
