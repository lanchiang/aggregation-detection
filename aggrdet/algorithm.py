...
class SumDetectionRowWise(luigi.Task):
        ...
        with self.input().open('r') as input_file:
            ...
            for line in tqdm(input_file):
                ...
                for number_format in file_dict['valid_number_formats']:
                    ...
                    collected_results = []
                    while True:
                        ar_cands_by_row = [(self.detect_proximity_aggregation_relations(forest, error_bound), forest) for forest in
                                           forests_by_rows]
                        ...
                        ar_cands_by_row = self.prune_conflict_ar_cands(ar_cands_by_row, axis=0)
                        # prune the candidates that do not appear in X% of all rows. X equals to satisfied_vote_ratio
                        # ar_cands_by_row = self.prune_occasional_ar_cands(ar_cands_by_row, self.satisfied_vote_ratio, axis=0)
                        if not any(ar_cands_by_row):
                            break
                        ...
                        # Todo: Reach threshold means not occasional, but it can still be not a real aggregation
                        for forest in forests_by_rows:
                            results_dict = forest.results_to_str('Sum')
                            for result_dict in results_dict:
                                collected_results.extend(result_dict)
                            # file_dict['aggregation_detection_result'] = collected_results
                    # results.append(file_dict)
                    # results_by_number_format[number_format] = results
    ...
    def prune_conflict_ar_cands(self, ar_cands_by_row, axis=0):
        ...
        for ar_index in satisfied_cands_index:
            ...
            non_conflicts = self.filter_conflict_ar_cands(ar_index, non_conflict_ar_cands_index.keys())
    ...
    def prune_occasional_ar_cands(self, ar_cands_by_row, satisfied_ratio, axis=0):
        """
    ...
    def eliminate_negative(self, table_value: np.ndarray):
    ...
    def build_aggr_relation_search_space(self, ar_cands: List[AggregationRelation]) -> set:
        ar_valid_combi = []
        self.ar_ss_recursive(ar_cands, AggregationRelationCombination(), ar_valid_combi)
    ...
    def ar_ss_recursive(self, allowed_ar_cands: List[AggregationRelation], ar_combi: AggregationRelationCombination,
        ...
        for index, ar_cand in enumerate(allowed_ar_cands):
            ...
            compatible_ar_cands = self.select_compatible_ar_cands(ar_cand, allowed_ar_cands[index + 1:])
            self.ar_ss_recursive(compatible_ar_cands, ar_combi, ar_combi_ss)
            ar_combi.remove_tail()

    def filter_conflict_ar_cands(self, ar_index, list_ar_cands_index):
    ...
    def select_compatible_ar_cands(self, ar: AggregationRelation, ar_cands: List[AggregationRelation]):
        # conflict rule 1: bidirectional aggregation
    ...
    def detect_proximity_aggregation_relations(self, forest: AggregationRelationForest, error_bound: float) -> list:
        """
...
class SumDetectionColumnWise(luigi.Task):
        ...
        with self.input().open('r') as input_file:
            ...
            for line in tqdm(input_file):
                ...
                for number_format in file_dict['valid_number_formats']:
                    ...
                    collected_results = []
                    while True:
                        ar_cands_by_column = [(self.detect_proximity_aggregation_relations(forest, error_bound), forest) for forest in
                                              forests_by_columns]
                        # get all non empty ar_cands
                        ...
                        ar_cands_by_column = self.prune_conflict_ar_cands(ar_cands_by_column, axis=1)
                        # prune the candidates that do not appear in X% of all rows. X equals to satisfied_vote_ratio
                        # ar_cands_by_column = self.prune_occasional_ar_cands(ar_cands_by_column, self.satisfied_vote_ratio, axis=1)
                        if not any(ar_cands_by_column):
                            break
    ...
    def prune_conflict_ar_cands(self, ar_cands_by_row, axis=0):
        ...
        # non_conflict_ar_cands = []
        # for k, v in non_conflict_ar_cands_index.items():
        #     non_conflict_ar_cands.append(v)
        # return non_conflict_ar_cands
    ...
    def detect_proximity_aggregation_relations(self, forest: AggregationRelationForest, error_bound: float) -> list:
        ...
        for index, root in enumerate(roots):
            try:
                aggregation_value = float(root.value)
            except Exception:
                continue
            ...
            is_equal = False
            for i in range(index + 1, len(roots)):
                # if this value is empty, allows to continue
                if is_empty_cell(roots[i].value):
                    aggregatee_cells.append(roots[i])
                    continue
                try:
                    ...
                    sum += aggregatee
                    aggregatee_cells.append(roots[i])
                    if abs(sum - aggregation_value) <= error_bound:
                        is_equal = True
                        break
            if is_equal and len(aggregatee_cells) > 1:
                ar = AggregationRelation(copy(root), tuple([copy(aee_cell) for aee_cell in aggregatee_cells]), Direction.FORWARD)
...
class Aggrdet(luigi.Task):
    ...
    def requires(self):
        ...
        # return SumDetectionRowWise(self.dataset_path)
        # return SumDetectionColumnWise(self.dataset_path)
